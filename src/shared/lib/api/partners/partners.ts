...
export interface PartnerMetrics {
  ...
  /** Account-enabled flag (unchanged — already an honest "enabled accounts" count, not engagement). */
  is_active_count: number;
  is_verified_count: number;
  /** New — engagement in the trailing 30 days. Label as "(30d)"; distinct from `is_active_count`. */
  active_partners_30d: number;
  /** New — the engagement complement of `active_partners_30d`. */
  inactive_partners_30d: number;
